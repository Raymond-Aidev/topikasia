...
export async function downloadTicket(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const fontBoldPath = path.join(__dirname, '..', '..', '..', '..', 'assets', 'fonts', 'NotoSansKR-Bold.ttf');
    ...
    const useFont = hasKoreanFont ? 'Korean' : 'Helvetica';
    const useBoldFont = hasKoreanFont && fs.existsSync(fontBoldPath) ? 'KoreanBold' : 'Helvetica-Bold';

    // ── 헤더 ──
    const pageWidth = doc.page.width - 100; // margin 50 * 2
    doc.rect(50, 50, pageWidth, 60).fill('#1565C0');
    doc.fill('#FFFFFF').font(useBoldFont).fontSize(24)
      .text('TOPIK', 70, 65, { continued: true })
      .fontSize(14).text('  IBT', { continued: false });
    doc.fill('#FFFFFF').font(useFont).fontSize(12)
      .text('Test of Proficiency in Korean', 70, 90);

    // ── 타이틀 ──
    doc.moveDown(2);
    doc.fill('#212121').font(useBoldFont).fontSize(22)
      .text('Admission Ticket', { align: 'center' });
    doc.font(useFont).fontSize(12).fill('#757575')
    ...
    const drawRow = (label: string, value: string, y: number) => {
      ...
      doc.fill('#616161').font(useBoldFont).fontSize(11).text(label, labelX, y + 2);
      doc.fill('#212121').font(useFont).fontSize(12).text(value, valueX, y + 1);
    };

    drawRow('Registration No.', registrationNumber, currentY);
    currentY += rowHeight;
    drawRow('Exam', reg.examName || '-', currentY);
    currentY += rowHeight;
    drawRow('Exam Type', reg.examType === 'TOPIK_I' ? 'TOPIK I' : 'TOPIK II', currentY);
    currentY += rowHeight;
    drawRow('Exam Date', examDate, currentY);
    currentY += rowHeight;
    drawRow('Name (EN)', reg.englishName || '-', currentY);
    currentY += rowHeight;
    drawRow('Date of Birth', birthDate, currentY);
    currentY += rowHeight;
    drawRow('Gender', reg.gender === 'MALE' ? 'Male' : 'Female', currentY);
    currentY += rowHeight;
    drawRow('Test Center', reg.venueName || '-', currentY);
    ...
    const noticeY = doc.y;
    doc.fill('#F57F17').font(useBoldFont).fontSize(10)
      .text('NOTICE', 70, noticeY);
    doc.fill('#795548').font(useFont).fontSize(9)
    ...
    // ── 푸터 ──
    doc.moveDown(3);
    doc.fill('#BDBDBD').font(useFont).fontSize(8)
