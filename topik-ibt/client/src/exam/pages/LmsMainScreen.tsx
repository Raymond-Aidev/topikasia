  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#F5F5F5', fontFamily: 'sans-serif', paddingTop: topPad }}>
      ...
      <div style={{
        ...
        <div style={{ fontSize: isMobile ? 28 : 40, fontWeight: 800, marginBottom: 12 }}>학습하기</div>
            ...
            {history.map(item => (
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12, flexWrap: 'wrap', gap: 8 }}>
                  <div>
                    <div style={{ fontSize: isMobile ? 15 : 17, fontWeight: 700, color: '#111827' }}>{item.examSetName}</div>
                  ...
                  {item.score && item.score.isPublished && (
                    <div style={{ textAlign: 'right' }}>
                      <div style={{ fontSize: isMobile ? 20 : 24, fontWeight: 800, color: '#4A148C' }}>
                ...
                {item.score && item.score.isPublished && (
                  <div style={{ display: 'flex', gap: 12, marginBottom: 12, flexWrap: 'wrap' }}>
                    {Object.entries(item.score.sectionScores).map(([section, data]: [string, any]) => (
                      <div key={section} style={{ flex: isMobile ? '1 1 calc(50% - 6px)' : 1, backgroundColor: '#FFFFFF', borderRadius: 8, padding: '8px 12px' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <span style={{ fontSize: 13, color: '#4A148C', fontWeight: 600 }}>
                    문제 복습 →
                  </span>
                </div>
              </div>
